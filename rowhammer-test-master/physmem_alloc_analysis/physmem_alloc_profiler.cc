...
uint64_t get_physical_frame_num(uintptr_t virtual_addr) {
  ...
  uint64_t pvalue;
	uint64_t testval;
	printf("value undefined int %x ", testval);
	printf("Value undefinede right? %x ", pvalue);
//	printf("poss?: %x ", &value);
  int got = read(fd, &pvalue, 8);
	int testgot = read(fd, &testval, 8);
	printf("Address of value: %x Vaule of value: %x ", &pvalue, pvalue);  
  assert(got == 8); 
  int rc = close(fd);
  assert(rc == 0); 

  uint64_t frame_num = pvalue & ((1ULL << 54) - 1); 
	printf("frame num: %llx ", frame_num);  
return frame_num;
}
